...
status: unverified              # needs-capture | unverified | experimental | broken | stable  — см. провайдер deepseek-web (unverified до живой записи)
...
- `omniproxy provider validate <id>` — Zod-валидация + проверка ссылок на transforms,
  профили отпечатка и фикстуры. **Реализовано.**
- `provider.schema.json` генерируется из Zod и подключается в IDE — правка декларации
  идёт с автодополнением и подсветкой ошибок. **Планируется, см. 01-monorepo визион.**
- `omniproxy provider diff <id>` — сравнение фактического ответа с фикстурой,
  вывод расхождения путей (§9, диффер схем). **Планируется.**
...
### 5.2 `module.json` — метаданные для обмена *(планируется)*
...
### 5.3 Политика совместимости *(планируется)*

`schemaVersion` — публичный контракт. Поддерживаются текущая и предыдущая мажорные
версии; `omniproxy provider migrate` *(планируется)* переписывает декларацию автоматически там, где
